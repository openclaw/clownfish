...
- Fix execution should provide Codex actual repo-discovery context before editing; repeated "no target repo changes" means tune `scripts/execute-fix-artifact.mjs` before replaying more jobs. In GitHub Actions, workspace-write Codex edits may need `sandbox_workspace_write.network_access=true` to avoid bwrap loopback failures.
