...
When a canonical PR exists, autonomous follow-through must not skip the maintainer loop. The required path is: review current PR state, clear security-sensitive concerns, inspect actionable review comments, inspect review-bot comments from Greptile, Codex, Asile, CodeRabbit, Copilot, and similar reviewers, address findings or mark them blocked, run Codex `/review`, address every Codex review finding, refactor to the narrowest safe change, run targeted validation, confirm changelog/credit, then only recommend merge after checks and review state are clean. For a clean exact head, authenticate the fresh canonical CI gate, rebuild and validate the synthetic merge on current `main`, wait for GitHub's matching test merge, and authorize that commit through the strict App-owned exact-merge check; rebase only when the effective diff or conflict state actually changes. After the PR lands, rerun duplicate classification against the landed PR/commit before recommending closeout.

Every merge action must carry `merge_preflight`. Missing security clearance, unresolved human or bot comments, missing/failed Codex `/review`, unaddressed findings, or missing validation commands blocks merge. Worker-authored preflight is planning input only: every executable merge is rebuilt by deterministic exact-head preflight, authenticated to canonical CI, authorized by the Clownfish App, and performed with that same non-bypassed App token. Set `require_external_merge_preflight: true` to route exact-head queue jobs there immediately. The fix executor runs the agentic prep loop before pushing: edit, validation, Codex `/review`, address findings, revalidate, then resolve review threads when `CLOWNFISH_RESOLVE_REVIEW_THREADS=1`. The applicator also checks live GitHub review threads immediately before squash merge.
