...
- optional `CLOWNFISH_EXTERNAL_PREFLIGHT_CODEX_SANDBOX` variable for the exact-merge Codex review. Local runs default to `read-only`; GitHub Actions defaults to `danger-full-access` because hosted Ubuntu runners reject Codex's bubblewrap loopback setup. The review subprocess receives no GitHub or API token, runs on an ephemeral checkout, and is followed by tracked-byte, tree, parent, and Git-config integrity verification.
