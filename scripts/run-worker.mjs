...
function repairResultIfNeeded() {
  for (let attempt = 1; attempt <= resultRepairAttempts; attempt += 1) {
    ...
    const repairPrompt = [
      ...
      "Security repair rule: `route_security` must target exact issue/PR refs only. Never emit `route_security` for `cluster:<cluster_id>`. If security-sensitive refs block a cluster-level fix lane, route the exact refs and replace the cluster-level fix action with cluster-scoped `needs_human` using `target_kind: null` and `target_updated_at: null`.",
