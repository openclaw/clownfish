...
- Treat numeric HTML entities in code, diffs, tests, comments, or titles as literals, not issue refs. Examples: `&#99999999;`, `&#x110000;`, and `&#128512;` are entity/code-point test data unless the same number is also a hydrated GitHub item.
...
- Do not emit `build_fix_artifact`, `open_fix_pr`, or planned `fix_needed` when any executable repair target or lineage ref is blocked by `clawsweeper:automerge` or a security `merge-risk:*` label, unless the job is an adopted automerge repair that explicitly allows that source. In explicit repair-only merge-risk remediation jobs (`allowed_actions` includes `fix` and `raise_pr`, `blocked_actions` includes `merge`, `allow_fix_pr: true`, and `allow_merge: false`), non-security `merge-risk:*` labels are repair scope, not a reason to return `keep_*` only. Keep non-target linked refs out of `fix_artifact.linked_refs` if they are not being repaired.
...
- The fix plan must be narrow: list only the files expected to change, focused tests, review-bot findings to address, and the exact branch/PR that could not be updated if applicable. For `repair_contributor_branch`, do not copy every file already touched by the source PR into `likely_files`; include only files the executor should edit or inspect to clear the current blocker. A review-comment-only repair should usually list the inline comment paths, not the full PR diff.
