...
- If security-sensitive refs block a cluster-level fix path, still emit `route_security` only for the exact issue/PR refs. For the cluster-level fix lane, emit a cluster-scoped `needs_human` action with `target: "cluster:<cluster_id>"`, `target_kind: null`, and `target_updated_at: null`; do not emit cluster-scoped `route_security`, and do not emit `build_fix_artifact` unless you include a complete executable `fix_artifact`.
