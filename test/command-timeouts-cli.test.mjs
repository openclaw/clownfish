...
const cases = [
  ...
  { script: "dispatch-jobs", hang: "git-fetch-backlog", dispatch: true,
    args: ["--wait-for-capacity", "--publish-backlog-wait-ms", "1000"] },
];
...
`, { mode: 0o755 });
    ...
    const child = spawnSync(process.execPath, args, {
      cwd: root, encoding: "utf8", timeout: 20000, killSignal: "SIGKILL",
      env: {
        ...
        CLOWNFISH_GH_EXEC_TIMEOUT_MS: "2000", HANG_KIND: scenario.hang, CALLS: calls,
      },
    });
    assert.equal(child.error, undefined, child.stderr);
    assert.equal(child.status === 0, Boolean(scenario.ok), child.stderr || child.stdout);
    assert.match(child.stderr, /timed out after (1000|[1-9][0-9]{0,2}|2000)ms/);
